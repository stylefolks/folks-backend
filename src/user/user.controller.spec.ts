...
describe('UserController', () => {
  ...
  it('getUser calls service with id', async () => {
    mockUserService.getProfileByUserName.mockResolvedValue({ id: '1' });

    const result = await userController.getUserByName('1');

    expect(mockUserService.getProfileByUserName).toHaveBeenCalledWith('1');
